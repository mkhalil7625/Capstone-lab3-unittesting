...
from studentList.studentlists import ClassList, StudentError
...
class TestStudentLists(TestCase):
    ...
    def test_removed_student(self):
        test_class = ClassList(3)
        test_class.add_student('Snoop Dogg')
        test_class.remove_student('Snoop Dogg')
        self.assertNotIn('Snoop Dogg' , test_class.class_list)
    ...
    def test_remove_student_not_in_list(self):
        test_class = ClassList(3)
        test_class.add_student('Snoop Dogg')
        test_class.add_student('Mo Khalil')
        with self.assertRaises(StudentError):
            test_class.remove_student('Mohammed Khalil')
    ...
    def test_is_not_enrolled(self):
        test_class = ClassList(3)
        test_class.add_student('Snoop Dogg')
        test_class.add_student('Mohammed Khalil')
        self.assertFalse(test_class.is_enrolled('Mo'))
    ...
    def test_index_of_student_return_none_empty_list(self):
        test_class = ClassList(2)
        self.assertIsNone(test_class.index_of_student('Mo'))
    ...
    def test_is_class_full_when_class_is_full(self):
        class_list = ClassList(2)
        class_list.add_student('gfajadg')
        class_list.add_student('cadcedfe')
        self.assertTrue(class_list.is_class_full())
    ## TODO write a test for your new is_class_full method for when is empty, and when it is not full. Use assertFalse
    def test_is_class_empty_when_class_is_full_false(self):
        class_list = ClassList(2)
        self.assertFalse(class_list.is_class_full())
